...
//   // This line creates a mutation function called addTeacher
...
//     try {
//       if (formType === 'teacher') {
//         //addTeacher send data to backend
...
export const Modall = ({ show, onHide, formType }) => {
  ...
  // Fetch any required data for the form
  const { data, loading, error } = useQuery(currentSchema.query || '');
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (currentSchema.mutation) {
        const res = await mutate({ variables: formData });
  ...
  const renderField = (field) => {
    const commonProps = {
      ...
      required: field.required,
      className: "py-2",
      disabled: isSubmitting || (field.select && loading),
    };
    ...
    <Modal show={show} onHide={onHide} centered backdrop="static">
      ...
      <Modal.Footer className="border-top-0 pt-0">
        ...
        <Button 
          style={{backgroundColor: "#1d3557"}}
          onClick={handleSubmit}
          disabled={isSubmitting || !currentSchema.mutation}
        >
          {isSubmitting ? (
            <>
              <Spinner
